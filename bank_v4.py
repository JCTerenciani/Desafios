...
class Cliente:
    ...
    def realizar_transacao(self, conta, transacao):
        ...
        numero_transacoes = len(list(conta.historico.transacoes_do_dia()))
...
class ContaCorrente(Conta):
    ...
    @classmethod
    def nova_conta(cls, cliente, numero, limite):
...
class Historico:
    ...
    def transacoes_do_dia(self):
        # 1. Acessa a lista de transações com 'self'
        # 2. Obtém a data de hoje, apenas o dia, mês e ano
        hoje = datetime.now().date()
        
        # 3. Itera sobre todas as transações na lista
        for transacao in self._transacoes:
            # 4. Converte a string da data da transação para um objeto de data
            data_transacao = datetime.strptime(transacao["data"], "%d-%m-%Y %H:%M:%S").date()
            
            # 5. Compara as datas. Se forem iguais, retorna a transação com 'yield'
